.data
Sorted_Array: .asciiz "Sorted Array: ["    # Prompt for sorted array
Space: .asciiz ", "                       # Comma and space separator
Bracket: .asciiz "]"                       # Closing bracket for the array
c: .word 0:100                             # int c[100] is global, initialized to 0
array: .word 1,4,7,10,25,3,5,13,17,21     # First input array
array2: .word 56,3,46,47,34,12,1,5,10,8,33,25,29,31,50,43  # Second input array

.text

Main:
    la $a0, array2      # Load address of array2 to $a0 as an argument
    addi $a1, $zero, 0  # $a1 = low
    addi $a2, $zero, 15 # $a2 = high
    jal Mergesort        # Call Mergesort function
    la $a0, Sorted_Array # Load address of prompt: "Sorted Array: ["
    li $v0, 4            # MIPS syscall for printing strings
    syscall
    jal Print            # Call Print function to print the sorted array
    la $a0, Bracket      # Load address of closing bracket for the array
    li $v0, 4            # MIPS syscall for printing strings
    syscall
...
If:
    sll $t0, $s0, 2       # $t0 = i*4
    add $t0, $t0, $a0     # add offset to the address of a[0]; now $t0 = address of a[i]
...
While2:
    ...
    lw $t1, 0($t0)        # load value of a[i] into $t1
